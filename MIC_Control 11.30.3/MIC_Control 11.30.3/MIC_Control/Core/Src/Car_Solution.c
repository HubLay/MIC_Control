...
#define Car_H 0.3
#define Car_W 0.3							//并没有一定要按车子实际来调
#define X_static 100
#define Y_static 100
...
void Move_Transfrom(float Vx,float Vz)
{
  ...
  M4.Expect_Speed= 44.3736f * (float)(Vx-Vz*Car_W/2);
	*/
	...
	Set_ExSpeed(&M1,44.3736f * (float)(Vx*Car_H-Vz*Car_W/2));	
	Set_ExSpeed(&M2,44.3736f * (float)(Vx*Car_H+Vz*Car_W/2));
	Set_ExSpeed(&M3,44.3736f * (float)(Vx*Car_H+Vz*Car_W/2));
	Set_ExSpeed(&M4,44.3736f * (float)(Vx*Car_H-Vz*Car_W/2));
