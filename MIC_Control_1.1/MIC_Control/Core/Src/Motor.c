...
void Struct_Init(float Kp, float Ki, float Kd)
{
    M1.Number = 1;
    M1.GPIOX_M = GPIOA;
...
// 设置实际速度
void Set_Speed(struct Motor *M)
{   
    //采用的PWM模式2，而不是PWM模式1，因为用的两个上拉电阻，高阻态输出
    uint16_t PWM = M->PWM > 0 ? M->PWM : -M->PWM; // abs(M->PWM*100)        0-100
    uint8_t flag = M->PWM > 0 ? 1 : 0;            // 设置电机正反转，根据实际改
...
void PI_Motor(struct Motor *M)
{
    ...
    M->error[0]   = M->Expect_Speed - M->Speed;
	...
	// 限幅
    if (M->PWM > 100) M->PWM = 100;
    if (M->PWM < -100) M->PWM = -100;	
    
	
    /*
	if(M->PWM <5 && M->PWM >-5) M->PWM = 0;			//计数器的偏差，速度在01波动时，Speed被滤波为0，
													//但实际上还会有很低速的转动，并且此时error一直为0，无法调速
